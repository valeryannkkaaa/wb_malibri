...
# sync rotate -> search report rotate -> funnel rotate -> optimizer -> parse positions -> stocks -> snapshots.
...
#   SKIP_SYNC / SKIP_SEARCH_REPORT / SKIP_FUNNEL / SKIP_PARSE / SKIP_STOCKS = 1 — пропустить шаг
...
if [ "${SKIP_SYNC:-0}" != "1" ]; then
    echo; echo "[1/7] Sync rotate (3 campaigns + fullstats if due)..."
    python -m scripts.sync_pilot --rotate --pause "$SYNC_PAUSE" --limit 3
fi

if [ "${SKIP_SEARCH_REPORT:-0}" != "1" ]; then
    echo; echo "[2/7] Search report rotate (1 nm_id)..."
    python -m scripts.sync_search_report --rotate --pause "$SYNC_PAUSE" --limit 1
fi

if [ "${SKIP_FUNNEL:-0}" != "1" ]; then
    echo; echo "[3/7] Funnel rotate (1 nm_id, 365d history)..."
    python -m scripts.sync_funnel --rotate --pause "$SYNC_PAUSE" --limit 1
fi

echo; echo "[4/7] Optimizer (suggest-only)..."
python -m scripts.run_optimizer

if [ "${SKIP_PARSE:-0}" != "1" ]; then
    echo; echo "[5/7] Parse positions..."
    ...
    python -m scripts.parse_positions "${PARSE_ARGS[@]}" || {
        echo "[5/7] Parse positions failed (best-effort), continuing..."
    }
fi

if [ "${SKIP_STOCKS:-0}" != "1" ]; then
    echo; echo "[6/7] Stocks report (skip if synced <24h)..."
    python -m scripts.sync_stocks
fi

echo; echo "[7/7] Capture snapshots (memory)..."
