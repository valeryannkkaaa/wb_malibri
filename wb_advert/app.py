...
from wb_advert.middleware import PortalSessionAuthMiddleware
...
def create_app() -> FastAPI:
    application = FastAPI(title="WB Advert Module", version="0.2.0")
    application.add_middleware(PortalSessionAuthMiddleware)
    session_secret = (settings.portal_session_secret or "").strip()
    if session_secret:
    ...
    application.include_router(advert_api_router, prefix="/api/advert", tags=["advert"])

    if (_PKG / "static").is_dir():
        application.mount("/static", StaticFiles(directory=str(_PKG / "static")), name="static")

    @application.get("/", response_class=HTMLResponse)
    @application.get("/advert", response_class=HTMLResponse)
    @application.get("/advert/", response_class=HTMLResponse)
    ...
    @application.get("/advert/decisions", response_class=HTMLResponse)
